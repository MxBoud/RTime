...
class reactionTimeGUI(Tkinter.Tk):
    ...
    def initialize(self):
        ...
        button2 = Tkinter.Button(self,text="Ajouter au presse-papier !",command = self.processToClipBoard)
        ...
        label1Variable.set("\n\n\nCliquez le boutton ÃÂ«Commencer l'expÃÂ©rienceÃÂ» pour dÃÂ©buter\n\n\n")
        ...
        self.table.insert("end",("1","Aucune donnée disponible"))
    ...
    def reactionTimeExp(self): #DÃÂ©but du reaction time Exp 
    ...
    def sortOfMainLoop(self): #fonction instaurÃÂ©e pour bypasser le mainloop pour que mon
        #code soit effectuÃÂ© ÃÂ  chaque fois que ce loop arrive.  
        ...
        while self.windowExist:
            if self.waitForStimulus==1:
                ...
                if time.time()>self.stimulusTime:
                    ...
                    #print time.time()
                    self.experimentButton.config(text = "APPUYEZ", bg = "green")
                    self.waitForStimulus = 0
                    self.waitForUser = 1
